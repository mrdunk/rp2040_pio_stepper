...
extern uint8_t axis_initialized_bitmask;
...
static void test_do_steps__period_not_set(void **state) {
    ...
    axis_initialized_bitmask |= 1 << _joint;

    // Disables all joints in config.
    expect_value(__wrap_disable_joint, joint, _joint);
    expect_value_count(__wrap_disable_joint, core, 1, 1);

    // Stops joint.
    expect_value(stop_joint, joint, _joint);

    uint32_t period = 0;
    uint16_t ret_val = __real_do_steps(_joint, period);
